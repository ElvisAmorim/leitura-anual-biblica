...
function showUnreadPreviousDays() {
    ...
    fetch("data/data.json")
        ...
        .then(tableData => {
            ...
            // Calcular o total de dias esperados até hoje
            const totalDaysExpected = todayDay; // Número de dias esperados até hoje no mês

            // Calcular os dias lidos até hoje
            const totalDaysRead = Object.values(savedStatus).filter(status => status === true).length;

            // Atualizar o texto no frontend
            const summaryElement = document.querySelector("#unread-summary");
            if (unreadCount === 0) {
                // Se os dias lidos forem maiores que os dias esperados, mostrar mensagem de adiantamento
                if (totalDaysRead > totalDaysExpected) {
